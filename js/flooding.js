...
function loadDay(dayNumber) {
  ...
  const ts        = `?t=${Date.now()}`;
  const pointsFP  = `data/WPC_Data/wpc_day_${dayNumber}.geojson${ts}`;
  const outlookFP = `data/WPC_Outlooks/WPCOutlook_day_${dayNumber}.geojson${ts}`;
